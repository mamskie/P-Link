<header class="navbar" x-data="{ open: false, atTop: false }"
    @if (request()->is('admin*')) :class="{ 'sticky top-0 z-50': atTop }"
        @scroll.window="atTop = (window.pageYOffset < 65) ? false: true" @endif>
    <div class="container-alt px-4 flex h-16 justify-between" :class="{ 'sm:hidden': atTop }">
        <a class="navbar-brand logo text-blue-400" href="{{ url('/') }}">
            <img src="{{ asset('images/logo.png') }}" alt="Logo" class="h-8 inline-block mr-2">
            {{ config('app.name') }}
        </a>
        ...
        <div class="flex items-center">
            ...
            <div class="-mr-2 ml-6 flex items-center sm:hidden">
                <button
                    class="navbar-toggler rounded-md
    ...
    <div class="navbar-mobile sm:hidden block" :class="{ 'block': open, 'hidden': !open }" x-show="open" x-transition
        {{-- Prevent blinking --}} style="display: none;">
        @auth
            @role('admin')
